...
densityPlizzleFunctizzle <- function(xlab, ylab, titl, genreVar, popularityVar, data) {
  cleanData <- data %>% 
    select(full_price, !!sym(genreVar), !!sym(popularityVar)) %>%
    filter(!is.na(!!sym(genreVar)), full_price <= 70, !!sym(popularityVar) <= 250)
...
densityPlizzle <- densityPlizzleFunctizzle("Price", "Density", "Price Density Meta", "Adventure", "meta_uscore", steamDataGenres)
densityPlizzle

densityPlizzle2 <- densityPlizzleFunctizzle("Price", "Density", "Price Density IGDB", "Adventure", "igdb_uscore", steamDataGenres)
densityPlizzle2

densityPlizzle3 <- densityPlizzleFunctizzle("Price", "Density", "Price Density steam", "Adventure", "store_uscore", steamDataGenres)
densityPlizzle3
```
...
# Correlation of different games (TODO)
```{r}
...
sources2 <- c("igdb_uscore", "meta_uscore", "store_uscore")
inputs2 <- expand_grid(sources, sources)

source_cor <- map2(inputs2$sources...1, inputs2$sources...2, source_stats) %>% 
  bind_rows()
```
...
score_avg <- steamDataGenres %>%
  mutate(gfq_rating = gfq_rating*20) %>% # scales gfq_rating to be on the 0-100 scale
  pivot_longer(cols = c(store_uscore, gfq_rating, meta_score, meta_uscore, igdb_score, igdb_uscore), names_to = "score_name") %>% 
  group_by(score_name) %>% 
  summarize(avg = mean(value, na.rm = TRUE)) %>% 
  arrange(desc(avg)) %>% 
  mutate(score_name = factor(score_name, levels = score_name)) %>% 
  mutate(score_type = ifelse(score_name %in% c("meta_score", "igdb_score", "gfq_rating"), "not_user_rating", "user_rating"))
